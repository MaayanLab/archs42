...
function downloadFile(termid, cid, species) {
    ...
    //$.getJSON("https://jsonip.com", function (data) {
        $.ajax({
            url : "search/downloadtracker.php?ip=notavailable&searchterm="+searchterm+"&samplenumber="+samp.length+"&species="+species,
            dataType: "text",
...
function selectHand(samples){
    ...
    if(samples.length > 0){
        ...
        if(activeMode == "sample"){
            ...
            var str = "<tr id=\"tr-"+termid+"\"><td style=\"padding: 0px;\"><div id=\""+termid+"-result\" style=\"padding:0px;\"><input type=\"text\" id=\""+termid+"\"></div></td><td>"+searchterm+"</td><td>"+(activeSpecies.charAt(0).toUpperCase() + activeSpecies.slice(1))+"</td><td>"+samples.length+"</td><td>NA</td><td style=\"font-size: 1.4em;\"><a style=\"cursor:pointer\" width=\"14px\" onclick=\"downloadScript('"+termid+"','"+colorID+"');\">R</a> </td><td style=\"font-size: 1.4em;\"><div id=\"TrashButton\" class=\"glyphicon glyphicon-remove\" style=\"cursor:pointer\" onclick=\"removeRow('"+termid+"');\"></div></td></tr>";
...
function addSampleResults(termid, searchterm, samples, useries){
    ...
    var str = "<tr id=\"tr-"+termid+"\"><td style=\"padding: 0px;\"><div id=\""+termid+"-result\" style=\"padding:0px;\"><input type=\"text\" id=\""+termid+"\"></div></td><td>"+searchterm+"</td><td>"+(activeSpecies.charAt(0).toUpperCase() + activeSpecies.slice(1))+"</td><td>"+samples.length+"</td><td>"+seriescount+"</td><td style=\"font-size: 1.4em;\"><a style=\"cursor:pointer\" width=\"14px\" onclick=\"downloadScript('"+termid+"','"+colorID+"');\">R</a> </td><td style=\"font-size: 1.4em;\"><div id=\"TrashButton\" class=\"glyphicon glyphicon-remove\" style=\"cursor:pointer\" onclick=\"removeRow('"+termid+"');\"></div></td></tr>";
    
    var re = new RegExp("^GSE[0-9]{5,12}$");
    if(re.test(searchterm)){
        str = "<tr id=\"tr-"+termid+"\"><td style=\"padding: 0px;\"><div id=\""+termid+"-result\" style=\"padding:0px;\"><input type=\"text\" id=\""+termid+"\"></div></td><td>"+searchterm+" <a target=\"_blank\" style=\"margin-left: 30px;\" href=\"series/"+searchterm+"\"><img src=\"images/biojupies.png\" width=12> BioJupies</a></td><td>"+(activeSpecies.charAt(0).toUpperCase() + activeSpecies.slice(1))+"</td><td>"+samples.length+"</td><td>NA</td><td style=\"font-size: 1.4em;\"><a style=\"cursor:pointer\" width=\"14px\" onclick=\"downloadScript('"+termid+"','"+colorID+"');\">R</a> </td><td style=\"font-size: 1.4em;\"><div id=\"TrashButton\" class=\"glyphicon glyphicon-remove\" style=\"cursor:pointer\" onclick=\"removeRow('"+termid+"');\"></div></td></tr>";
    }
